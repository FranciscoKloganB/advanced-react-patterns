...
In this simple exercise I learned how to create a better API for the consumers of
a custom Context Provider, by not forcing them to implement their state change
functions.

Not only does this provide a better development experience, it also, sometimes,
increases performance. The performance gain only would exist, if the the Context
Provider included functions (with or without React.useCallback) within its closure.
However, the developer experience gain will always be existent.
