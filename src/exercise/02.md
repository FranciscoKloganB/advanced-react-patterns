...
In this lesson I learned how to implicitly share state of a parent component
with its direct children. To this, we need to clone the children of the parent
component from within the parent component and adding extra props to them.

This pattern is called Compound Components and is most useful when it makes
sense to group components where parents do not make sense without the children
and vice-versa.

E.g.: In the example below a `<select>` tag does not make sense without
`<option>` tags.

As a side note we also learned how to selectively choose to what we pass
props and how to handle native html tags within Compound Components.
