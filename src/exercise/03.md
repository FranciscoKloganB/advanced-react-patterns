...
In this lesson we learned how to leverage custom `useHooks` with Context provider
to implement the previous pattern, with a simpler approach and which can be
used to implicitly share the state with to compound children at any depth.

This use case will be the one used most often, since the other one only makes sense
if and only if, business logic or intended API usage of the component only
intends to provide state for direct children of the root component of the
