...
In this modules we learned on how to create custom components that allow its
users to gain control of explicit state management, if they desire it. This can
be useful for example, when we want to run specific business logic code when
a change on the state occurs (e.g.: similar to Vue's watch method) or when we
want to synchronize the state between multiple components under a given parent
component. Not exactly the same as in the previous module -- In module five we
learned how to share state implicitly between the children of a component. Here
we are talking about performing state changes in the children of the component
depending on state changes that happened on some child component. E.g.: having
two buttons with the same `on` state.

As a bonus, we created a custom hook to ensure that errors that can derive from
using Component controlled properties are caught during development, by providing
meaningful debug messages through the reusable hook.
